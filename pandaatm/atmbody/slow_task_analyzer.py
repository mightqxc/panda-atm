...
class SlowTaskAnalyzer(AgentBase):
    ...
    def __init__(self):
        ...
        self.jobWaitMaxHours = 16
        self.goodJobRunMaxHours = 96
        self.badJobRunMaxHours = 16
        # director to dump report files
    ...
    def _bad_job_qualify(self, job_attr_dict):
        ...
        wait_long = False
        # long wait
        if job_attr_dict['waitDuration'] > datetime.timedelta(hours=self.jobWaitMaxHours):
            wait_long = True
        # according to status
        if job_attr_dict['jobStatus'] == 'finished':
            if wait_long:
                # finished job waiting too long
                retVal = True
                ret_msg_set.add('GoodJobWaitLong')
            if job_attr_dict['runDuration'] > datetime.timedelta(hours=self.goodJobRunMaxHours):
                # finished job running for too long
                retVal = True
                ret_msg_set.add('GoodJobRunLong')
        else:
            # failed, cancelled, closed
            if wait_long:
                # bad job waiting long, might have gotten stuck in some status
                retVal = True
                ret_msg_set.add('BadJobWaitLong')
            if job_attr_dict['runDuration'] > datetime.timedelta(hours=self.badJobRunMaxHours):
                # wasted time cost by bad job running
                retVal = True
                ret_msg_set.add('BadJobRunLong')
        return retVal, ret_msg_set

    def _bad_jobs_display(self, pandaid_list, job_slow_reason_set, err_info_dict) -> str:
        sorted_err_info_list = sorted(err_info_dict.items(), key=(lambda x: (x[1]['n_jobs'], x[1]['waitDuration'] + x[1]['runDuration'])), reverse=True)
        errors_str = '\n    '.join([ '{n_jobs:>6} | {avg_wait:>12} | {avg_run:>12} | {info}'.format(
                                            n_jobs=x[1]['n_jobs'],
        ...
        result_str = (
                '  Reasons: {reasons_str}\n'
                '  Jobs: {pandaids_str}\n'
                '     NJobs |   AvgWaiting |   AvgRunning |   jobStatus |            computingSite | Dialogs \n'
                '    {errors_str}\n'
            ).format(
                    reasons_str=','.join(sorted(job_slow_reason_set)),
        ...
        while True:
            ...
            with open(report_file, 'w') as dump_file:
                ...
                dump_str = (
                            ...
                            'jobWaitMaxHours = {jobWaitMaxHours}\n'
                            'goodJobRunMaxHours = {goodJobRunMaxHours}\n'
                            'badJobRunMaxHours = {badJobRunMaxHours}\n'
                            '\n'
                            ).format(
                                    ...
                                    jobWaitMaxHours=self.jobWaitMaxHours,
                                    goodJobRunMaxHours=self.goodJobRunMaxHours,
                                    badJobRunMaxHours=self.badJobRunMaxHours,
                                )
                ...
                for k in sorted(ret_dict):
                    ...
                    # culprit jobs
                    pandaid_list = []
                    job_slow_reason_set = set()
                    err_info_dict = {}
                    for jobspec in jobspec_list:
                        job_attr_dict = self._get_job_attr_dict(jobspec)
                        retVal, ret_msg_set = self._bad_job_qualify(job_attr_dict)
                        if retVal:
                            # qualified bad job
                            pandaid_list.append(jobspec.PandaID)
                            job_slow_reason_set |= ret_msg_set
                    ...
                    if n_bad_jobs == 0:
                        tmp_log.debug('taskID_attempt={0} got 0 culprit jobs'.format(task_attempt_name))
                    else:
                        bad_jobs_display_str = self._bad_jobs_display(pandaid_list, job_slow_reason_set, err_info_dict)
                        slow_reason_set |= job_slow_reason_set
                        dump_str = 'taskID_attempt={0} got {1} culprit jobs: \n{2}\n\n'.format(task_attempt_name, n_bad_jobs, bad_jobs_display_str)
                        dump_file.write(dump_str)
                        tmp_log.debug(dump_str)
